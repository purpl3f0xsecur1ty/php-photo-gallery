...
if (
    ...
    if ((isset($_GET['filename'])) && (preg_match("/^[^\/\"'<>*]+$/", $_GET['filename']))) {
        ...
        if ($previous_file !== false) {
            $html_content .= '<div id="previous" class="p"><a class="button" href="viewer.php?category=' . $requested_category . '&filename=' . $previous_file . '">&lt;</a></div>';
        }
        if ($next_file !== false) {
            $html_content .= '<div id="next" class="p"><a class="button" href="viewer.php?category=' . $requested_category . '&filename=' . $next_file . '">&gt;</a></div>';
        }

        $html_content .= '<img src="' . $path_to_file . '" alt="' . $requested_file . '">';
        $html_action_controls = '<div id="action_controls"><ul>
<li><a class="button2" href="index.php?category=' . $requested_category . '">Back</a></li>
</ul>
